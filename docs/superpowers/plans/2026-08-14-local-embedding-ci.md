...
**Goal:** 以本地 `BAAI/bge-small-zh-v1.5` 替换无效的 DeepSeek Embedding 调用，并为 Node/Python 服务添加无密钥测试和 GitHub Actions 验证。
...
- [ ] **Step 2: 运行失败测试，确认失败原因是 `LocalEmbeddings` 未定义**
...
- [ ] **Step 1: 写失败测试，定义 Node 到 Python 的健康契约**
